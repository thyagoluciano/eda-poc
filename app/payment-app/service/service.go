...
func (s service) Consumer(message *domain.Message) error {
	...
	paymentCmd := domain.PaymentCmd{
		Id: payload.Id,
		PaymentAttempt: domain.PaymentAttempt{
			PaymentId: domain.GenerateId(),
			Status:    "IN_PROGRESS",
			Price: domain.Price{
				Amount:   payload.Payment.Price.Amount,
				Currency: payload.Payment.Price.Currency,
				Scale:    payload.Payment.Price.Scale,
			},
		},
	}

	if payload.Payment.Token == "1" {
		paymentCmd.Status = "COMPLETED"
	} else {
		paymentCmd.Status = "FAILED"
	}

	s.Producer(ctx, paymentCmd)
...
func (s service) Producer(ctx context.Context, cmd domain.Command) error {
	...
	logger := log.With(s.logger, "method", "INVENTORY_APP")
