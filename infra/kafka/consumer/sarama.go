...
type GroupHandler struct {
	f func(message *domain.Message) error
}
...
func (sarama SaramaConsumer) Subscribe(f func(message *domain.Message) error) {
	go sarama.readMessages(f)
}

func (sarama SaramaConsumer) readMessages(f func(message *domain.Message) error) {
	for {
