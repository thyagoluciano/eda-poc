...
func (segmentio SegmentioConsumer) Subscribe(f func(message *domain.Message) error) {
	go segmentio.readMessages(f)
}

func (segmentio SegmentioConsumer) readMessages(f func(message *domain.Message) error) {
	for {
